...
- **单一事实源**：关卡文案在 `src/essay/training.ts`（目标、核心要点、实操任务、自评清单、docId），
  客观题库在 `src/essay/bank.ts`（每关 6 道四选一），判星规则在 `src/essay/rules.ts`。
  前端不重复打包一份文案（`GET /api/essay/training` 把定义与进度一起下发），接口校验也用同一份定义。
- **通关口径**：客观题 + 自评清单的加权总分。客观题**每题 2 分**（客观可验证，权重更高），
  自评清单**每条 1 分**（校验「任务真的做了」）；满分 20 分，≥80% 一星（通关）、≥90% 二星、满分三星
  ——与专项训练「一局 10 题准确率」同一套 80/90/100 分界。另有**硬门槛**：客观题答对不足 80%
  （5 题即 <4 题）时，自评勾满也不通关。星级由服务端按提交内容重算（不接受客户端传来的星数），
  且**只增不减**（重新交卷答得更差不会抹掉已得的星）。
- **客观题**：每关题库 6 道、下发 5 道，题序与选项顺序都按 `levelId` **确定性打乱**（同一关每次打开题面一致，
  可续答、可回看，但与题库原文顺序不同）；选项带题库**原始下标 `key`**，前端提交的是 `key`，所以展示顺序
  打乱不影响判分。**下发数据里没有答案与解析**（字段名换写法也不行），交卷后由 `marks` 回传每题对错、
  正确答案与解析，页面据此渲染错题回顾。未答完的提交返回 400 `QUIZ_INCOMPLETE`。
- **逐关解锁**：第一关是入口，其余关卡要求上一关已通关（`essayUnlocked`）；前端锁住提交按钮，
  服务端在 `POST /api/essay/training/:levelId` 再判一次，未解锁返回 409 `LEVEL_LOCKED`。
- **进度存储**：`essay_training_progress`（`migrations/0006` + `0007`），`level_id` 是键——**关卡 id 一经
  发布不要改**；自评勾选只存下标、客观题只存作答（`[{id,key}]`），对错与解析由题库现算，避免「库里存着旧解析」；
  老数据（只有自评那批）的 `quiz_json` 是默认 `'[]'`，读取时视为「客观题未作答」，星级不受影响。
- **加内容**：先在指南里挑出「目标 / 要点 / 任务 / 清单」，写进 `ESSAY_LEVELS`（每关清单固定 10 条，
...
- **本地 mock 的加载约束**：`scripts/dev-api-server.mjs` 用纯 node 直接 `import` 上面这几个源码文件，
  ...
  `tests/essay-training.test.ts` 里有这条守卫——不要为了方便把规则再导出回 `training.ts`。
