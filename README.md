...
- **单一事实源**：`src/essay/training.ts`。关卡目标、核心要点、实操任务、自评清单、docId 都写在这里，
  前端不重复打包一份文案（`GET /api/essay/training` 把定义与进度一起下发），接口校验也用同一份定义。
- **通关口径**：申论没有客观题可自动判分，所以通关依据是「做完实操任务 → 对照清单自评 → 留下笔记」。
  星级按**清单勾中比例**算：≥80% 一星（通关）、≥90% 二星、全中三星——与专项训练「一局 10 题准确率」
  同一套分界；星级由服务端按清单条数重算（不接受客户端传来的星数），且**只增不减**（改自评不会抹掉已得的星）。
- **逐关解锁**：第一关是入口，其余关卡要求上一关已通关（`essayUnlocked`）；前端锁住提交按钮，
  服务端在 `POST /api/essay/training/:levelId` 再判一次，未解锁返回 409 `LEVEL_LOCKED`。
- **进度存储**：`essay_training_progress`（见 `migrations/0006_essay_training.sql`），
  `level_id` 是键——**关卡 id 一经发布不要改**；自评勾选只存下标（数组 JSON），清单文案改动不会污染历史。
- **加内容**：先在指南里挑出「目标 / 要点 / 任务 / 清单」，再写进 `ESSAY_LEVELS`（每关清单固定 10 条，
  星级阈值才有干净分界），延伸讲解挂到 essay 知识库既有专文即可；`tests/essay-training.test.ts` 会核
  关卡数、阶段区间、清单条数、docId 是否存在、解锁链与星级口径。
