...
category: profit
title: "经济利润问题"
summary: "区分成本、售价、利润和利润率，折扣通常作用于标价。"
order: 30
...
先分清成本、标价、售价：售价=`标价×折扣`，利润=`售价-成本`，利润率=`利润÷成本`——分母永远是成本，不要误写成售价。题中没有具体数值时优先赋值，把价格设为10或100，便于计算百分数与折扣。
...
**2、解题方法**：设未知数，可列出一元二次方程进行求解。一般列出的方程都是两点式，即y=(a-x)(b+x)，求出使方程等于0，解得𝑥₁、𝑥₂，求出两个𝑥的平均值𝑥=（𝑥₁+𝑥₂）÷2时，此时y是最值。
...
**例1**：(2018江西) 小李四年前投资的一套商品房价格上涨了50%，由于担心房价下跌，将该商品房按市价的9折出售，扣除成交价5% 的相关交易费用后，比买进时赚了56.5万元。那么，小李买进该商品房时花了多少万元 ?（ ）
...
解析
   ...
   设买进该商品房时成本为 x 万元，则现在定价为1.5x 万元，实际售价为1.5x×0.9=1.35x 万元。利润(售价-成本)：(1-5%)×1.35x-x=56.5，解得 x=200。故正确答案为 A
...
解析
   ...
   设产品提高了x个档次，每日获得总利润为y，则每件的利润为8+2x元，每日售出的数量为60−5x，那么每天获得的利润y=(8+2x)(60−5x)。当y=0时，x=−4或12，若让每天获得的利润y最大，此时x应取−4与12的平均值(−4+12)/(2)=4，此时y=(8+2×4)(60−5×4)=640。故正确答案为C。
...
**例6**：(2018国考)枣园每年产枣2500公斤，每公斤固定盈利18元。为了提高土地利用率，现决定明年在枣树下种植紫薯 ( 产量最大为10000公斤 )，每公斤固定盈利3元。当紫薯产量大于400公斤时，其产量每增加 n 公斤将导致枣的产量下降0.2n 公斤。问该枣园明年最多可能盈利多少元 ?（ ）
...
解析
   ...
   当紫薯产量大于400公斤时，每增加 n 公斤将导致枣的产量下降0.2n 公斤。假设紫薯的产量为 (400+n) 公斤，则此时枣的产量为 (2500-0.2n) 公斤。则总盈利为18×(2500-0.2n)+3×(400+n)=(46200-0.6n) 元，要让总盈利46200-0.6n最大，则 n 取0，此时总盈利为46200元。故正确答案为 B。
...
解析
   ...
   方法二：根据题意，每天获利最高时是一定满足大于等于350元的情况，又因为选项范围各不相同，因此可验证每天获利最高时的售价。设该商品售价提高了x元，每天获利为y元。根据题意，可列方程：y=（10-8+x）（100-10x），令y=0，解得𝑥₁=-2，𝑥₂=10，当（-2+10）÷2=4时，每天获利y最大为（2+4）×（100-10×4）=360>350，满足条件，此时商品售价为10+4=14元，在B项范围内。
   方法三：设该商品售价提高了x元，为保证每天至少获利350元，根据题意，可列方程：（10-8+x）（100-10x）≥350，化简可得（x-3）（x-5）≤0，，则当3≤x≤5时满足题干要求，即该商品售价应在10+3=13元和10+5=15元之间。
...
解析
   ...
   方法一：赋值法
   赋上月单件礼品的利润为10，则本月的单利为10×（1-20%）=8。设上月销售量为a，本月销售量为x，根据公式：总利润=单利×总销量。
   则10a（1+36%）=8x，x=1.7a，故本月销售量较上月增加了(x-a)/a=(1.7a-a)/a=70%。故正确答案为D。
   方法二：增长率法
   总利润=单利×总销量。求总销量的增长率。
   总销量=总利润÷单利。根据比值增长率a=(b-c)/(1+c)
   代入总销量的增长率=（36%+20%）/1-20%=70%。故正确答案为D。
