...
> **本篇口径**：每个技巧都先给**口算路线**（秒杀法，能不动笔就不动笔）；带 ✏️ 标注的是需要动笔的老实算法，只作**参考与对照**，考场上不要用。
...
✏️常规法（参考·考场不用）：分别算出三种债券的基期量，然后求占比，计算麻烦
...
✏️常规法（参考·考场不用）：依次计算出2018年、2019年公开量，计算量较大
...
**验算对照**：精确值 B＝459.5、X＝−27.5——分配法得到的 460 只差 0.5，误差可以忽略（这行只用来检验准确度，考场上不必算）。
...
**验算对照**：精确值 B＝679.1、X＝−61.1——分配法得到的 679、−61 与它一致，两种分配都可用（同样只作检验，不用在考场上算）。
...
**验算对照**：精确值 B＝3156.5、X＝−978.5——分配法得到的 3157.6、−979.6 差不到 1，说明「剩余量太小直接归入基期」这个简化是安全的（同样只作检验）。
